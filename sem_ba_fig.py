...
xmax = meanv.max() + 0.002
ax.text(xmax, bias + 0.012, f"bias = {bias:+.4f} \u00b5m", ha="right",
        color="crimson", fontsize=8.5)
ax.text(xmax, loa_hi + 0.010, f"+1.96 SD = {loa_hi:+.4f} \u00b5m", ha="right",
        color="0.25", fontsize=8.5)
ax.text(xmax, loa_lo - 0.014, f"\u22121.96 SD = {loa_lo:+.4f} \u00b5m", ha="right",
        color="0.25", fontsize=8.5)
ax.text(xmax, bias - 0.055, f"n = {len(diff)} pairs\npaired t-test p = {p:.2f}",
        ha="right", va="top", color="0.25", fontsize=8)
ax.set_xlim(right=xmax + 0.004)
